...
contract ViperGeyser is IStaking, Ownable{
     ...
     *  startBonus_ - Starting time bonus, BONUS_DECIMALS fixed point.
     ...
     */
    constructor(IERC20Upgradeable stakingToken, IERC20Upgradeable distributionToken, uint256 maxUnlockSchedules,
     ...
     */
    function getDistributionToken() public view returns (IERC20Upgradeable) {
        assert(_unlockedPool.token() == _lockedPool.token());
        return _unlockedPool.token();
    }

    /**
     *  Transfers amount of deposit tokens from the user, which would be locked for 21 days.
    ...
    /**
     *  Transfers amount of deposit tokens from the caller on behalf of user.
    ...
    /**
     *  Private implementation of staking methods.
     *  staker - User address who deposits tokens to stake.
     *  beneficiary - User address who gains credit for this stake operation.
     *  amount - Number of deposit tokens to stake.
     */
    ...
    /**
     *  Unstakes a certain amount of previously deposited tokens if the minimum staking period is completed(21 days). User also receives their
     *  alotted number of distribution tokens after the lockedpool by the owner gets  unlocked.
    ...
    /**
     *  Unstakes a certain amount of previously deposited tokens. User also receives their
    ...
    function _unstake(uint256 amount) private returns (uint256) {
        ...
        // interactions
        require(stakeTimeSec > 1814400,"TokenGeyser: current time is before release time")// the token would be locked for 21days
        require(_stakingPool.transfer(msg.sender, amount);
            'TokenGeyser: transfer out of staking pool failed');
        if(_unlockedPool.balance > 0){
    ...
    /**
     *      Applies an additional time-bonus to a distribution amount. This is necessary to
     *      encourage long-term deposits instead of constant unstake/restakes.
     *      The bonus-multiplier is the result of a linear function that starts at startBonus and
     *      ends at 100% over bonusPeriodSec, then stays at 100% thereafter.
     *  currentRewardTokens - The current number of distribution tokens already alotted for this
     *                         unstake op. Any bonuses are already applied.
     *  stakingShareSeconds - The stakingShare-seconds that are being burned for new
     *                            distribution tokens.
     *  stakeTimeSec - Length of time for which the tokens were staked. Needed to calculate
     *                     the time-bonus.
    ...
    /**
     *  Note that this application has a staking token as well as a distribution token, which
    ...
    /**
     *  A globally callable function to update the accounting state of the system.
    ...
    /**
     *  This funcion allows the contract owner to add more locked distribution tokens, along
    ...
    function lockTokens(uint256 amount, uint256 durationSec) external onlyOwner{ 
        durationSec = 94608000;// the tokens would be locked for 3yrs 
    ...
    /**
     *  Moves distribution tokens from the locked pool to the unlocked pool, according to the
    ...
    /**
     *  Returns the number of unlockable shares from a given schedule. The returned value
     *      depends on the time since the last unlock. This function updates schedule accounting,
     *      but does not actually transfer any tokens.
     *  s - Index of the unlock schedule.
